...
async function rollOneSave(nameOrActor, ab) {
  ...
  try {
    ...
    try { if (game.dice3d && roll) await game.dice3d.showForRoll(roll, game.user, true); } catch (e) {}   // AWAIT so the 3D dice actually land before the save result is marked/applied
...
async function featureWeaponMastery(card, message) {
  try {
    ...
    const autoMastery = (() => { try { return game.settings.get(NS, 'featureMasterySaveAuto'); } catch (e) { return false; } })();   // follows its OWN toggle, NOT Full Auto — so you can run full-auto combat yet still be prompted to roll Topple etc. yourself (with visible dice)
